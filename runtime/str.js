...
matchesAt__ink_qm__ = (s, substring, idx) => (() => {let max = len(substring); return (() => {let sub = i => {return __ink_match(i, [[() => max, () => true], [() => __Ink_Empty, () => __ink_match((() => {let __ink_acc_trgt = __as_ink_string(s); return __is_ink_string(__ink_acc_trgt) ? __ink_acc_trgt.valueOf()[(() => {return __as_ink_string(idx + i)})()] || null : (__ink_acc_trgt[(() => {return __as_ink_string(idx + i)})()] !== undefined ? __ink_acc_trgt[(() => {return __as_ink_string(idx + i)})()] : null)})(), [[() => (() => {return (() => {let __ink_acc_trgt = __as_ink_string(substring); return __is_ink_string(__ink_acc_trgt) ? __ink_acc_trgt.valueOf()[(() => {return i})()] || null : (__ink_acc_trgt[(() => {return i})()] !== undefined ? __ink_acc_trgt[(() => {return i})()] : null)})()})(), () => sub(__as_ink_string(i + 1))], [() => __Ink_Empty, () => false]])]])}; return sub})()(0)})();
module.exports.matchesAt__ink_qm__ = matchesAt__ink_qm__;
index = (s, substring) => (() => {let max = (len(s) - 1); return (() => {let sub = i => {return __ink_match(matchesAt__ink_qm__(s, substring, i), [[() => true, () => i], [() => false, () => __ink_match((i < max), [[() => true, () => sub(__as_ink_string(i + 1))], [() => false, () => __ink_negate(1)]])]])}; return sub})()(0)})();
...
replaceNonEmpty = (s, old, __ink_ident_new) => (() => {let lold = len(old); let lnew = len(__ink_ident_new); return (() => {let sub = (acc, i) => {return __ink_match(matchesAt__ink_qm__(acc, old, i), [[() => true, () => sub(__as_ink_string(__as_ink_string(slice(acc, 0, i) + __ink_ident_new) + slice(acc, __as_ink_string(i + lold), len(acc))), __as_ink_string(i + lnew))], [() => false, () => __ink_match((i < len(acc)), [[() => true, () => sub(acc, __as_ink_string(i + 1))], [() => false, () => acc]])]])}; return sub})()(s, 0)})();
...
splitNonEmpty = (s, delim) => (() => {let coll = []; let ldelim = len(delim); return (() => {let sub = (acc, i, last) => {return __ink_match(matchesAt__ink_qm__(acc, delim, i), [[() => true, () => (() => {(() => {let __ink_assgn_trgt = __as_ink_string(coll); __is_ink_string(__ink_assgn_trgt) ? __ink_assgn_trgt.assign(len(coll), slice(acc, last, i)) : (__ink_assgn_trgt[len(coll)]) = slice(acc, last, i); return __ink_assgn_trgt})(); return sub(acc, __as_ink_string(i + ldelim), __as_ink_string(i + ldelim))})()], [() => false, () => __ink_match((i < len(acc)), [[() => true, () => sub(acc, __as_ink_string(i + 1), last)], [() => false, () => (() => {let __ink_assgn_trgt = __as_ink_string(coll); __is_ink_string(__ink_assgn_trgt) ? __ink_assgn_trgt.assign(len(coll), slice(acc, last, len(acc))) : (__ink_assgn_trgt[len(coll)]) = slice(acc, last, len(acc)); return __ink_assgn_trgt})()]])]])}; return sub})()(s, 0, 0)})();
...
trimPrefixNonEmpty = (s, prefix) => (() => {let max = len(s); let lpref = len(prefix); let idx = (() => {let sub = i => {return __ink_match((i < max), [[() => true, () => __ink_match(matchesAt__ink_qm__(s, prefix, i), [[() => true, () => sub(__as_ink_string(i + lpref))], [() => false, () => i]])], [() => false, () => i]])}; return sub})()(0); return slice(s, idx, len(s))})();
...
trimSuffixNonEmpty = (s, suffix) => (() => {let lsuf = len(suffix); let idx = (() => {let sub = i => {return __ink_match((i > __ink_negate(1)), [[() => true, () => __ink_match(matchesAt__ink_qm__(s, suffix, (i - lsuf)), [[() => true, () => sub((i - lsuf))], [() => false, () => i]])], [() => false, () => i]])}; return sub})()(len(s)); return slice(s, 0, idx)})();
